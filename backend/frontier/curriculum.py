...
from backend.repro.determinism import deterministic_timestamp, deterministic_timestamp_from_content
...
def validate_curriculum_config(config: Dict[str, Any]) -> List[str]:
    """
    Validate a curriculum configuration dictionary.
    ...
    if version is None:
        ...
        errors.append(f"unsupported version {version}, expected 2")

    # Systems check
    systems = config.get("systems")
    if not isinstance(systems, dict):
        errors.append("missing or invalid 'systems' field")
        return errors  # Can't continue without systems

    for slug, system_cfg in systems.items():
        errors.extend(_validate_system(slug, system_cfg))

    return errors


def _validate_system(slug: str, cfg: Dict[str, Any]) -> List[str]:
    """Validate a single system configuration."""
    errors: List[str] = []
    prefix = f"system '{slug}'"
    ...
    if not isinstance(slices, list) or not slices:
        errors.append(f"{prefix}: missing or empty 'slices' list")
        return errors

    active = cfg.get("active")
    slice_names = set()

    for idx, slice_cfg in enumerate(slices):
        slice_errors, name = _validate_slice(slug, idx, slice_cfg)
        errors.extend(slice_errors)
        if name:
            if name in slice_names:
                errors.append(f"{prefix}: duplicate slice name '{name}'")
            slice_names.add(name)

    if active and active not in slice_names:
        errors.append(f"{prefix}: active slice '{active}' not found in slices")

    # Monotonicity check
    invariants = cfg.get("invariants", {})
    mono_axes = invariants.get("monotonic_axes", [])
    if mono_axes:
        errors.extend(_validate_monotonicity(slug, slices, mono_axes))

    return errors


def _validate_slice(system_slug: str, idx: int, cfg: Dict[str, Any]) -> Tuple[List[str], Optional[str]]:
    """Validate a single slice configuration. Returns (errors, slice_name)."""
    errors: List[str] = []
    prefix = f"system '{system_slug}' slice[{idx}]"
    ...
    if not isinstance(params, dict):
        errors.append(f"{prefix}: missing or invalid 'params'")

    gates = cfg.get("gates")
    if not isinstance(gates, dict):
        errors.append(f"{prefix}: missing or invalid 'gates'")
    else:
        for gate_name in ("coverage", "abstention", "velocity", "caps"):
            if gate_name not in gates:
                errors.append(f"{prefix}: missing gate '{gate_name}'")

    return errors, name


def _validate_monotonicity(slug: str, slices: List[Dict[str, Any]], axes: List[str]) -> List[str]:
    """Validate monotonicity invariants across slices."""
    errors: List[str] = []
    prior: Optional[Dict[str, int]] = None

    for idx, slice_cfg in enumerate(slices):
        params = slice_cfg.get("params", {})
        current: Dict[str, int] = {}
        for axis in axes:
            value = params.get(axis)
            if value is None:
                errors.append(f"system '{slug}' slice[{idx}]: missing monotonic axis '{axis}'")
            else:
                try:
                    current[axis] = int(value)
                except (TypeError, ValueError):
                    errors.append(f"system '{slug}' slice[{idx}]: axis '{axis}' must be integer")

        if prior is not None:
            for axis in axes:
                if axis in current and axis in prior:
                    if current[axis] < prior[axis]:
                        errors.append(
                            f"system '{slug}' slice[{idx}]: monotonicity violation on '{axis}': "
                            f"{prior[axis]} -> {current[axis]}"
                        )
        prior = current
...
# ---------------------------------------------------------------------------
# Wave 2: FOL Gating Placeholders
# ---------------------------------------------------------------------------


def make_fol_entry_slice() -> CurriculumSlice:
    """
    Construct the entry slice for First-Order Logic (FOL) curriculum.

    Wave 2 placeholder: this slice will be activated once the PL ladder
    is complete and the organism demonstrates stable coverage in propositional
    logic. FOL introduces quantifiers and predicates, requiring instantiation
    and background knowledge.

    Thresholds are intentionally conservative for the initial FOL transition.
    """
    gates = SliceGates(
        coverage=CoverageGateSpec(
            ci_lower_min=0.88,  # Lower than PL to allow exploration
            sample_min=30,      # More samples needed for FOL complexity
            require_attestation=True,
        ),
        abstention=AbstentionGateSpec(
            max_rate_pct=25.0,  # Higher tolerance for FOL abstentions
            max_mass=1500,
        ),
        velocity=VelocityGateSpec(
            min_pph=80.0,       # Slower proofs expected in FOL
            stability_cv_max=0.20,
            window_minutes=90,
        ),
        caps=CapsGateSpec(
            min_attempt_mass=4000,
    ...
    return CurriculumSlice(
        ...
        params=params,
        gates=gates,
        metadata={'wave': 2, 'ladder_position': 'fol_base'},
    )


def make_fol_equality_slice() -> CurriculumSlice:
    """
    Construct the FOL with equality slice (FOL{=}).

    Wave 2 placeholder: introduces equality reasoning, rewriting, and
    congruence closure. This is the bridge between FOL and equational logic.
    """
    gates = SliceGates(
        coverage=CoverageGateSpec(
            ci_lower_min=0.86,
    ...
    params = {
        'quantifier_depth': 3,
        'predicate_arity_max': 4,
        'term_depth_max': 5,
        'equality_chains_max': 6,
        'breadth_max': 2000,
        'total_max': 12000,
    }
    return CurriculumSlice(
        ...
        params=params,
        gates=gates,
        metadata={'wave': 2, 'ladder_position': 'fol_equality'},
    )
...
def load(system_slug: str, *, validate: bool = True) -> CurriculumSystem:
    """
    Load curriculum configuration and return a CurriculumSystem dataclass.

    Args:
        system_slug: System identifier (e.g., 'pl', 'fol')
        validate: If True, run structured validation before parsing

    Raises:
        FileNotFoundError: If curriculum.yaml is missing
