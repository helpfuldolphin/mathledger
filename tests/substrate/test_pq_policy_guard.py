"""
PQ hash policy guard tests covering epoch rules and health summaries.
"""

from analysis.global_health import build_pq_policy_tile
...
def test_synthetic_chain_end_to_end_summary():
    policy = _policy()
    headers = [
        BlockHeaderPQ(3, "sha256", False, True),
        BlockHeaderPQ(14, "sha3-256", False, True),
        BlockHeaderPQ(23, "sha3-256", True, False),
    ]
    verdicts = [validate_block_pq_policy(header, policy) for header in headers]

    assert verdicts[0]["ok"] is True
    assert verdicts[1]["code"] == "DUAL_COMMITMENT_REQUIRED"
    assert verdicts[2]["code"] == "LEGACY_HASH_REQUIRED"

    tile = build_pq_policy_tile(verdicts)

    assert tile["status"] == "alert"
    assert tile["violation_count"] == 2
    assert tile["latest_epoch"] == "pq_only"
    assert tile["latest_block"] == 23
    assert sorted(tile["violation_codes"]) == ["DUAL_COMMITMENT_REQUIRED", "LEGACY_HASH_REQUIRED"]
    assert "violations" in tile["headline"]
