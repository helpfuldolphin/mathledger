...
3. LeanExecutor capability detection
4. LeanExecutor explicit opt-in for stub
5. Canonical sort order for deterministic processing

9 tests total (3 new tests for hardening).
...
from backend.u2.lean_executor_hardened import (
    LeanCapability,
    LeanExecutorReal,
    LeanExecutorStub,
    create_lean_executor,
    create_executor,
)
...
def test_lean_capability_detection():
    """
    Test Lean capability detection.
    """
    print("\n[TEST 6] Lean Capability Detection")
    
    is_installed = LeanCapability.is_lean_installed()
    version = LeanCapability.get_lean_version()
    stub_allowed = LeanCapability.is_stub_allowed()
    
    print(f"  Lean installed: {is_installed}")
    print(f"  Lean version: {version or 'N/A'}")
    print(f"  Stub allowed: {stub_allowed}")
    print("  ✓ Capability detection working")


# ============================================================================
# TEST 7: LEAN EXECUTOR STUB REQUIRES EXPLICIT OPT-IN
# ============================================================================

def test_lean_executor_stub_requires_opt_in():
    """
    Test that LeanExecutorStub requires explicit opt-in.
    """
    print("\n[TEST 7] LeanExecutorStub Requires Explicit Opt-In")
    
    # Save original env var
    original_value = os.environ.get("U2_LEAN_ALLOW_STUB")
    
    try:
        # Disable stub
        os.environ.pop("U2_LEAN_ALLOW_STUB", None)
        
        # Try to create stub (should fail)
        try:
            stub = LeanExecutorStub()
            assert False, "LeanExecutorStub should require explicit opt-in"
        except RuntimeError as e:
            assert "U2_LEAN_ALLOW_STUB=1" in str(e)
            print("  ✓ Stub correctly rejects without opt-in")
        
        # Enable stub
        os.environ["U2_LEAN_ALLOW_STUB"] = "1"
        
        # Try to create stub (should succeed)
        stub = LeanExecutorStub()
        print("  ✓ Stub correctly accepts with opt-in")
        
    finally:
        # Restore original env var
        if original_value is not None:
            os.environ["U2_LEAN_ALLOW_STUB"] = original_value
        else:
            os.environ.pop("U2_LEAN_ALLOW_STUB", None)


# ============================================================================
# TEST 8: LEAN EXECUTOR FACTORY WITH CAPABILITY DETECTION
# ============================================================================

def test_lean_executor_factory():
    """
    Test create_lean_executor factory with capability detection.
    """
    ...
    try:
        ...
        # Create executor (should use real if Lean installed, stub otherwise)
        executor = create_lean_executor(allow_stub=True)
        
        if LeanCapability.is_lean_installed():
            assert isinstance(executor, LeanExecutorReal)
            print(f"  ✓ Factory created LeanExecutorReal (Lean installed)")
        else:
            assert isinstance(executor, LeanExecutorStub)
            print(f"  ✓ Factory created LeanExecutorStub (Lean not installed, stub allowed)")
...
# ============================================================================
# TEST 9: LEAN EXECUTOR FACTORY FAILS WITHOUT LEAN AND WITHOUT STUB
# ============================================================================

def test_lean_executor_factory_fails_without_lean_and_stub():
    """
    Test that factory fails with clear message when Lean is not installed
    and stub is not allowed.
    """
    print("\n[TEST 9] Lean Executor Factory Fails Without Lean and Stub")
    
    # Skip if Lean is installed
    if LeanCapability.is_lean_installed():
        print("  ⊘ Skipped (Lean is installed)")
        return
    ...
    try:
        ...
        # Try to create executor (should fail with clear message)
        try:
            executor = create_lean_executor(allow_stub=False)
            assert False, "Factory should fail when Lean not installed and stub not allowed"
        except RuntimeError as e:
            assert "Lean 4 is not installed" in str(e)
            assert "U2_LEAN_ALLOW_STUB=1" in str(e)
            print("  ✓ Factory correctly fails with clear error message")
...
def main():
    """Run all tests."""
    print("=" * 60)
    print("U2 BRIDGE LAYER TEST SUITE (HARDENED)")
    print("=" * 60)
    ...
    tests = [
        ...
        test_lean_capability_detection,
        test_lean_executor_stub_requires_opt_in,
        test_lean_executor_factory,
        test_lean_executor_factory_fails_without_lean_and_stub,
    ]
    ...
    if skipped > 0:
        print(f"         {skipped}/{len(tests)} tests skipped")
    if failed > 0:
