...
Walks artifacts/policy directories, reads policy.manifest.json, and updates
the head pointer (policy.json) along with optional policy_settings table entries.
"""
...
def main() -> None:
    ...
    parser.add_argument("--apply", action="store_true", help="Write head pointer + update DB/policy_settings.")
    ...
    for meta in manifests.values():
        print(f"[policy-backfill] discovered hash={meta.hash} version={meta.version} created_at={meta.created_at}")
...
def _determine_head_hash(root: Path, manifests: Dict[str, PolicyMetadata]) -> Optional[str]:
    ...
    if pointer_path.exists():
        try:
            ...
            pass
    # fallback: pick most recent created_at
...
def _update_policy_settings(db_url: str, policy_hash: str, policy_version: Optional[str]) -> None:
    ...
    try:
        ...
        print(f"[policy-backfill] DB update failed: {exc}")
