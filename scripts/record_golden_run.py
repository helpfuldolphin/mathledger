...
This tool executes a supplied experiment command with deterministic environment
guards, hashes the resulting HT-series log and trace log, snapshots the
environment, and emits a structured JSON receipt suitable for replay
validation.

Usage:
    # Note the `--` separating recorder args from the experiment command
    uv run python scripts/record_golden_run.py \
        --run-id u2-baseline \
        --ht-log results/fo_rfl_50.jsonl \
        --trace-log traces/u2_run.jsonl \
        --metrics artifacts/u2_metrics.json \
        -- python experiments/run_uplift_u2.py --slice first --cycles 10 ...
"""
...
def record_golden_run(args: argparse.Namespace) -> Path:
    """Entry point shared by CLI/tests."""
