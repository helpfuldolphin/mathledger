#!/usr/bin/env python3
"""
Generate v0.2.7 Verifier with Cryptographic Parity

This script creates the v0.2.7 evidence pack verifier with proper
domain-separated Merkle tree construction that matches Python.

v0.2.7 fixes the critical bug where v0.2.6 was deployed with a stale
verifier that lacked domain separation, causing u_t_mismatch failures.

Usage:
    uv run python scripts/generate_v027_verifier.py
"""
...
OUTPUT_PATH = REPO_ROOT / "site" / "v0.2.7" / "evidence-pack" / "verify" / "index.html"

# Get current git commit for embedding
...
<title>Evidence Pack Verifier - MathLedger v0.2.7</title>
...
<div class="container">
<div class="banner">
<div style="font-weight:600">MathLedger - v0.2.7</div>
<div><span class="status">LOCKED</span> <a href="/versions/" style="font-size:0.8rem;color:#666">(see /versions/)</a></div>
<div style="font-size:0.8rem;color:#555;margin-top:0.5rem">Tag: <code>v0.2.7-verifier-parity</code> | Commit: <code>__COMMIT_HASH__</code></div>
</div>
<nav class="nav"><a href="/v0.2.7/">Archive</a> <a href="/v0.2.7/evidence-pack/">Evidence</a> <strong>Verifier</strong> | <a href="/versions/">All</a></nav>
<h1>Evidence Pack Verifier</h1>
<div class="info"><strong>v0.2.7: Verifier Parity Fix</strong> - Fixes critical bug in v0.2.6 where deployed verifier lacked domain separation. JS now computes byte-for-byte identical hashes to Python.</div>
...
<footer>MathLedger v0.2.7 Verifier | <a href="../examples.json">Test Vectors (examples.json)</a> | Generated from commit <code>__COMMIT_HASH__</code></footer>
</div>
<script>
// v0.2.7: Verifier Parity Fix - Domain separation now matches Python
// GENERATED_FROM_COMMIT: __COMMIT_HASH__
...
def main():
    ...
    # Embed commit hash
    content = HTML_TEMPLATE.replace("__COMMIT_HASH__", COMMIT_HASH)
    ...
    print(f"OK: Written v0.2.7 verifier to {OUTPUT_PATH}")
