#!/usr/bin/env python3
"""
CAL-EXP-4: Variance Stress Experiment — Harness

BINDING REFERENCE: docs/system_law/calibration/CAL_EXP_4_VARIANCE_STRESS_SPEC.md
IMPLEMENTATION REFERENCE: docs/system_law/calibration/CAL_EXP_4_IMPLEMENTATION_PLAN.md
FREEZE REFERENCE: docs/system_law/calibration/CAL_EXP_4_FREEZE.md

Extends CAL-EXP-3 canonical harness with:
- Variance profile pre-registration
- Temporal structure audit artifact
- Variance profile audit artifact
- F5.x status in RUN_METADATA.json
...
IMPL_REFERENCE = "CAL_EXP_4_IMPLEMENTATION_PLAN.md"
FREEZE_REFERENCE = "CAL_EXP_4_FREEZE.md"
...
# Frozen thresholds per CAL_EXP_4_FREEZE.md §2
...
# =============================================================================
# TOOLCHAIN FINGERPRINT (inherited from CAL-EXP-3)
# =============================================================================
...
# =============================================================================
# SYNTHETIC CORPUS (inherited from CAL-EXP-3, with variance profile)
# =============================================================================
...
# =============================================================================
# ARM EXECUTION (inherited from CAL-EXP-3)
# =============================================================================
...
# =============================================================================
# TEMPORAL STRUCTURE AUDIT (NEW for CAL-EXP-4)
# =============================================================================
...
def generate_temporal_structure_audit(
    ...
    return {
        ...
        "_meta": {
            "generator": "run_cal_exp_4_harness.py",
            "note": "Temporal structure audit per CAL_EXP_4_FREEZE.md",
        },
    }


# =============================================================================
# VARIANCE PROFILE AUDIT (NEW for CAL-EXP-4)
# =============================================================================
...
def generate_variance_profile_audit(
    ...
    # Claim capping logic per FREEZE §4.2
    ...
    return {
        ...
        "_meta": {
            "generator": "run_cal_exp_4_harness.py",
            "note": "Variance profile audit per CAL_EXP_4_FREEZE.md",
        },
    }


# =============================================================================
# CAL-EXP-3 INHERITED ANALYSIS
# =============================================================================
...
def compute_uplift_report(
    ...
    """Compute ΔΔp per CAL-EXP-3 spec (inherited)."""
...
def compute_f5_status(
    ...
    """Aggregate F5.x failure codes per FREEZE §3."""
    f5_codes = []
...
def run_experiment(
    ...
    """Execute CAL-EXP-4 for a single pre-registered seed."""
    run_id = f"cal_exp_4_seed{seed}_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}"
    ...
    # Step 11: NEW — Temporal structure audit
    ...
    # Step 11: NEW — Variance profile audit
    ...
    # Step 13: Windowed analysis (inherited)
    ...
    run_metadata = {
        ...
        "claim_cap_level": f5_status["claim_cap_level"],
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="CAL-EXP-4: Variance Stress Experiment (Harness)"
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/cal_exp_4"),
        help="Output directory (default: results/cal_exp_4)",
    )
    parser.add_argument(
        "--noise-scale",
        type=float,
...
def main() -> int:
    ...
    print(f"f5_all_pass: {result['f5_all_pass']}")
