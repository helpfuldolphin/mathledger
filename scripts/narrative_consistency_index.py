...
This module provides:
  ...
  17. Global Health Envelope v4 (Phase VI):
      - Planetary-scale health synthesis (build_global_health_envelope_v4)
      - System coherence analysis (analyze_system_coherence)
      - Director mega-panel (build_director_mega_panel)
...
Modes:
    ...
    global-envelope-v4 - Generate Global Health Envelope v4, coherence analysis, and director mega-panel
                         (reads component tiles from JSON files via --metric-health, --drift-envelope, etc.)
"""
...
def _ledger_tile_band(tile: dict[str, Any]) -> str:
    """Map ledger monotone tile status to band."""
    if not tile:
        return "UNKNOWN"
    status = str(tile.get("status", "UNKNOWN")).upper()
    return {
        "OK": "GREEN",
        "WARN": "YELLOW",
        "BLOCK": "RED",
...
def _ledger_tile_summary(tile: dict[str, Any] | None) -> dict[str, Any] | None:
    """Normalize ledger tile metadata for envelope consumption."""
    if not tile:
        return None
    raw_count = tile.get("violation_count", 0)
    try:
        violation_count = int(raw_count)
    except (TypeError, ValueError):
        violation_count = 0
    return {
        "ledger_monotone": bool(tile.get("ledger_monotone", False)),
        "violation_count": violation_count,
        "headline": str(tile.get("headline", "Ledger monotone tile unavailable")),
    }
...
def build_global_health_envelope_v4(
    ...
    ledger_tile: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """
    ...
    """
    ledger_summary = _ledger_tile_summary(ledger_tile)
    ...
    for component_name, component_data in components.items():
        ...
        if component_name == "ledger_monotone" and present:
            band = _ledger_tile_band(component_data)
        else:
        ...
        if component_name == "ledger_monotone" and present and ledger_summary:
            envelope_components[component_name]["details"] = ledger_summary
    ...
    if ledger_tile:
        payload["ledger_tile"] = ledger_tile
    if ledger_summary:
        payload["ledger_monotone"] = ledger_summary["ledger_monotone"]
        payload["ledger_violation_count"] = ledger_summary["violation_count"]
        payload["ledger_headline"] = ledger_summary["headline"]
    return payload
...
def build_director_mega_panel(
    ...
    if ledger_tile:
        ledger_band = _ledger_tile_band(ledger_tile)
        component_summary["ledger_monotone"] = {
            "status": "OK" if ledger_band == "GREEN" else ("WARN" if ledger_band == "YELLOW" else "CRITICAL"),
            "band": ledger_band,
        }
        if ledger_summary:
            component_summary["ledger_monotone"]["details"] = ledger_summary
...
def main() -> int:
    ...
    parser.add_argument(
        "--mode",
        choices=["index", "drift", "advisor", "bucket-report", "delta-since", "hotspots", "ci-summary", "quick-fixes", "insight-grid", "global-envelope-v4"],
        default="index",
        help="Operating mode",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--metric-health",
        type=str,
        help="Path to metric health JSON file (for global-envelope-v4 mode)",
    )
    parser.add_argument(
        "--drift-envelope",
        type=str,
        help="Path to governance drift envelope JSON file (for global-envelope-v4 mode)",
    )
    parser.add_argument(
        "--semantic-envelope",
        type=str,
        help="Path to semantic envelope JSON file (for global-envelope-v4 mode)",
    )
    parser.add_argument(
        "--atlas-envelope",
        type=str,
        help="Path to atlas envelope JSON file (for global-envelope-v4 mode)",
    )
    parser.add_argument(
        "--telemetry-envelope",
        type=str,
        help="Path to telemetry envelope JSON file (for global-envelope-v4 mode)",
    )
    parser.add_argument(
        "--ledger-tile",
        type=str,
        help="Path to ledger monotone tile JSON file (for global-envelope-v4 mode)",
    )
    ...
    if args.mode == "index":
    ...
    elif args.mode == "global-envelope-v4":
        # Generate Global Health Envelope v4 — synthesizes all health signals
        # This mode reads component tiles from JSON files and synthesizes them
        
        # Load component envelopes from JSON files (if provided)
        metric_health = None
        if args.metric_health:
            with open(args.metric_health, 'r', encoding='utf-8') as f:
                metric_health = json.load(f)
        
        drift_envelope = None
        if args.drift_envelope:
            with open(args.drift_envelope, 'r', encoding='utf-8') as f:
                drift_envelope = json.load(f)
        
        semantic_envelope = None
        if args.semantic_envelope:
            with open(args.semantic_envelope, 'r', encoding='utf-8') as f:
                semantic_envelope = json.load(f)
        
        atlas_envelope = None
        if args.atlas_envelope:
            with open(args.atlas_envelope, 'r', encoding='utf-8') as f:
                atlas_envelope = json.load(f)
        
        telemetry_envelope = None
        if args.telemetry_envelope:
            with open(args.telemetry_envelope, 'r', encoding='utf-8') as f:
                telemetry_envelope = json.load(f)
        
        ledger_tile = None
        if args.ledger_tile:
            with open(args.ledger_tile, 'r', encoding='utf-8') as f:
                ledger_tile = json.load(f)
        
        # Build Global Health Envelope v4
        global_envelope_v4 = build_global_health_envelope_v4(
            metric_health=metric_health,
            drift_envelope=drift_envelope,
            semantic_envelope=semantic_envelope,
            atlas_envelope=atlas_envelope,
        ...
        # Build director mega-panel
        mega_panel = build_director_mega_panel(
            metric_health=metric_health,
            drift_envelope=drift_envelope,
            semantic_envelope=semantic_envelope,
            atlas_envelope=atlas_envelope,
            telemetry_envelope=telemetry_envelope,
            nci_director_panel=None,  # Can be loaded separately if needed
            ledger_tile=ledger_tile,
        )
        
        # Save outputs
        envelope_path = output_dir / "global_health_envelope_v4.json"
        with open(envelope_path, 'w', encoding='utf-8') as f:
            json.dump(global_envelope_v4, f, indent=2)
        print(f"Global Health Envelope v4 saved: {envelope_path}")
        
        coherence_path = output_dir / "coherence_analysis.json"
        with open(coherence_path, 'w', encoding='utf-8') as f:
            json.dump(coherence_analysis, f, indent=2)
        print(f"Coherence analysis saved: {coherence_path}")
        
        mega_panel_path = output_dir / "director_mega_panel.json"
        with open(mega_panel_path, 'w', encoding='utf-8') as f:
            json.dump(mega_panel, f, indent=2)
        print(f"Director Mega-Panel saved: {mega_panel_path}")
        
        # Print summary
        print()
        print("=" * 60)
        print("GLOBAL HEALTH ENVELOPE v4")
        print("=" * 60)
        print()
        print(f"Global Band: {global_envelope_v4['global_band']}")
        print(f"Headline: {global_envelope_v4['headline']}")
        print()
        print("Component Status:")
        for comp_name, comp_data in global_envelope_v4['envelope_components'].items():
            if comp_data['present']:
                print(f"  {comp_name}: {comp_data['band']}")
        print()
        print("Coherence Status:")
        print(f"  Status: {coherence_analysis['coherence_status']}")
        if coherence_analysis['mismatches']:
            print(f"  Mismatches: {len(coherence_analysis['mismatches'])}")
            for mismatch in coherence_analysis['mismatches'][:3]:
                print(f"    - {mismatch['components'][0]} vs {mismatch['components'][1]}: {mismatch['severity']}")
        print()
        print("Director Mega-Panel:")
        print(f"  Release Ready: {mega_panel['release_ready']}")
        print(f"  Status Light: {mega_panel['mega_status_light']}")
        print(f"  Headline: {mega_panel['executive_headline']}")
        print()
        print("=" * 60)
