...
def _resolve_database_url() -> str:
    """Resolve database URL with strict mode enforcement."""
    # Check explicit env overrides first
    explicit = os.environ.get("DATABASE_URL_TEST") or os.environ.get("DATABASE_URL")
    if explicit:
        return explicit
    ...
    if is_strict_mode():
        raise RuntimeError(
            "[STRICT] DATABASE_URL or DATABASE_URL_TEST must be set in strict mode."
        )

    # Non-strict: allow empty (will fail at connection time)
    return ""


def _resolve_redis_url() -> str:
    """Resolve Redis URL with strict mode enforcement."""
...
class RFLConfig:
    ...
    # Database (lazy lookup; env override via DATABASE_URL_TEST takes precedence)
    ...
    # Redis (lazy lookup)
    redis_url: str = field(
        default_factory=lambda: _resolve_redis_url()
    )
    ...
    def resolve_slice(self, run_index: int) -> CurriculumSlice:
        ...
        for slice_cfg in self.curriculum:
            if slice_cfg.contains(run_index):
                return slice_cfg
        # Fallback to last slice if nothing matched (should not happen after validation)
...
if __name__ == "__main__":
    ...
    print("RFL Configuration:")
    print(f"  Experiment ID: {config.experiment_id}")
    print(f"  Num runs: {config.num_runs}")
    print(f"  Derive steps per run: {config.derive_steps}")
    print(f"  Coverage threshold: {config.coverage_threshold}")
    print(f"  Uplift threshold: {config.uplift_threshold}")
    print(f"  Bootstrap replicates: {config.bootstrap_replicates}")
    print(f"  Artifacts dir: {config.artifacts_dir}")

    # Export to JSON
    config.to_json("config/rfl/example_config.json")
    print("\nConfiguration exported to config/rfl/example_config.json")
