...
class ModusPonensBenchmark(unittest.TestCase):
    ...
    def test_modus_ponens_performance_100_atoms(self):
        ...
        try:
            from axiom_engine.rules import ModusPonens, Statement
            def legacy_modus_ponens_wrapper(statements: Set[str]) -> Set[str]:
                premises = [Statement(text=s) for s in statements]
                # The legacy apply method returns a list of Statement objects
                derived_statements = ModusPonens.apply(premises)
                return {s.text for s in derived_statements}

            statements = self.generate_synthetic_dataset(100)
            result = self.measure_performance(legacy_modus_ponens_wrapper, statements, iterations=50)
    ...
    def test_modus_ponens_performance_1k_atoms(self):
        ...
        try:
            ...
            result = self.measure_performance(legacy_modus_ponens_wrapper, statements, iterations=10)
    ...
    def test_modus_ponens_performance_10k_atoms(self):
        ...
        try:
            ...
            result = self.measure_performance(legacy_modus_ponens_wrapper, statements, iterations=3)
    ...
    def tearDown(self):
        ...
        if self.results:
            ...
            existing_data.update(self.results)

            canonical_payload = json.dumps(existing_data, sort_keys=True, separators=(',', ':'), ensure_ascii=True)
